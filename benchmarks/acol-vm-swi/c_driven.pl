:- module(c_driven, [prepare_c_int/7,
                     ...
                     init/0,
                     linear_bytecode/7]).
:- use_module(assert_bc, [bc_to_terms/4]).
...
%foreign: pl_transform_list(+BCList, -BCPtr)
...
c_int(AST, Env, NewEnv) :-
    ...
    list_to_char_ptr(ResBCEnv, CEnvBC),
    list_to_char_ptr(ResBC, CBC),
...
test(while) :-
    ...
    linear_bytecode(AST, t, _, Len, ResBC, _, _), !, 
...
test(prepare_empty) :-
    prepare_c_env(t, t, 0, X), !, X == [].

test(prepare_nonempty) :-
    list_to_assoc([1-y, 0-x], IdAVL),
    list_to_assoc([x-int(2), y-int(7)], Env),
...
test(nonempty) :-
    empty_assoc(Empty),
    put_assoc(0, Empty, x, A1),
    put_assoc(1, A1, y, A2),
    put_assoc(2, A2, z, IdTable),

    put_assoc(x, Empty, int(20), B1),
    put_assoc(y, B1, int(21), B2),
    put_assoc(z, B2, int(22), Env),
...
test(nonempty) :-
    ...
    put_assoc(0, Empty, x, A1),
    put_assoc(1, A1, y, A2),
    put_assoc(2, A2, z, IdTableDec),

    put_assoc(x, Empty, int(20), B1),
    put_assoc(y, B1, int(21), B2),
    put_assoc(z, B2, int(22), Env),

    put_assoc(x, Empty, 0, C1),
    put_assoc(y, C1, 1, C2),
    put_assoc(z, C2, 2, IdTableEnc),
    env_to_bc(Env, IdTableEnc, IdTableDec, EnvBCLen, ResBC), !,
    ground(EnvBCLen),
