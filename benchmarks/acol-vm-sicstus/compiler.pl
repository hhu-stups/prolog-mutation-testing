...
:- use_module(objspace, [int_to_varname/3]).
...
h_idlist_to_idtable([H|T], AccEncode, AccDecode, Nr, ResEncode, ResDecode) :-
    Nr1 is Nr + 1,
    avl_store(H, AccEncode, Nr, NewAccEncode),
    avl_store(Nr, AccDecode, H, NewAccDecode),
...
store_subbytecode(AVL, SBC, Id, NewAVL) :-
    avl_size(AVL, Id),
    avl_store(Id, AVL, SBC, NewAVL).
retrieve_subbytecode(AVL, Id, SBC) :-
    avl_fetch(Id, AVL, SBC).
...
test(to_idtable) :-
    ...
    avl_fetch(x, REncode, 2),
    avl_fetch(z, REncode, 1),
    avl_fetch(y, REncode, 0),
    avl_size(REncode, 3),
    avl_fetch(2, RDecode, x),
    avl_fetch(1, RDecode, z),
    avl_fetch(0, RDecode, y),
    avl_size(RDecode, 3).
...
test(assignment) :-
    ...
    Res = [23, 2, 40, y, 200, 45, x], SBC == empty.
